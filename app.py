# -*- coding: utf-8 -*-
"""
Interest Rate Derivatives Presentation Generator – Streamlit App
"""
...
DGS10_GRAPH_PLACEHOLDER_KEY = "DGS10_GRAPH"  # {{DGS10_GRAPH}} on slide 3

# Placeholders that we EXPECT to be missing from DATA (handled as images)
GRAPH_PLACEHOLDERS = {SOFR_GRAPH_PLACEHOLDER_KEY, DGS10_GRAPH_PLACEHOLDER_KEY}
        ...
        for shape in iter_all_shapes(slide):
            if getattr(shape, "has_text_frame", False) and shape.has_text_frame:
                for p in shape.text_frame.paragraphs:
                    used |= {m.group(1).strip() for m in PLACEHOLDER_RE.finditer(paragraph_full_text(p))}
...
def collect_unresolved(prs: Presentation) -> dict:
    ...
    for si, slide in enumerate(prs.slides, start=1):
        ...
        for shape in iter_all_shapes(slide):
            if getattr(shape, "has_text_frame", False) and shape.has_text_frame:
                for p in shape.text_frame.paragraphs:
                    keys |= {m.group(1).strip() for m in PLACEHOLDER_RE.finditer(paragraph_full_text(p))}
            if getattr(shape, "has_table", False) and shape.has_table:
                for row in shape.table.rows:
                    for cell in row.cells:
                        for p in cell.text_frame.paragraphs:
...
def generate_pptx(
    ...
    """Run the whole pipeline and return the path to the generated PPTX."""
    ...
    if template_path.suffix.lower() != ".pptx":
        raise ValueError("Template must be .pptx")

    prs = Presentation(str(template_path))
    out_path = build_output_path(data)

    used_keys = collect_used_keys(prs)
    # Exclude graph placeholders from "missing" warnings
    missing = sorted(
        k for k in used_keys
        if k not in data and k not in GRAPH_PLACEHOLDERS
    )
    if missing:
...
def main():
    ...
    with st.form("input_form"):
        ...
        with col2:
            ...
            accrual_basis = st.text_input("Interest Accrual Basis", DATA_DEFAULT["Interest Accrual Basis"])
    ...
    if submitted:
        ...
        data.update({
            ...
            "Interest Accrual Basis": accrual_basis,
